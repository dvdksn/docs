...
To satisfy this target, add the corresponding target to the Dockerfile. The
`test` stage here is based on the same base stage as the build stage.
...
program, using matrices. Using matrices lets you run parallel builds with
different configurations, saving time and ensuring consistency.
...
In the `docker-bake.hcl` file, create a new `binaries` stage similar to the
`default` stage that builds images. You can omit the `tags` and `attest`
attributes since they're only relevant for Docker images. Instead, this stage
should set the `output` attribute to export the results to the filesystem.
...
This target assumes that there's a `binaries` stage in the Dockerfile that
contains the binaries that you want to export. The contents of that stage are
exported to the local filesystem path `./build/${mode}/`, where `mode` is the
expanded matrix variable, e.g. `./build/debug/`.

Next, add the `binaries` stage to the Dockerfile:
...
Now, running `docker buildx bake binaries` builds all of the variants of your
program and exports them to the local `build/` directory. Subdirectories are
automatically created for platform variants.
...
Generating all the variants at once is nice for release purposes, but for local
development and testing, maybe you just want a quick way to build a single
binary for your local platform. Buildx has a special `local` platform value for
that.

Add another Bake target for generating and exporting only the local platform
binary:
...
buildx bake localbin`.

By default, when `platforms` is unspecified, builds target the architecture of
the BuildKit host, typically running on Linux. If you're using Docker Desktop,
this often means builds target `linux/amd64` or `linux/arm64`, even if your
local machine is macOS or Windows. Using the `local` platform forces the target
platform to match your local environment.

For example, on macOS, the `localbin` target generates an executable in the
[Mach-O format](https://en.wikipedia.org/wiki/Mach-O) — the standard executable
format for macOS.

```console
$ docker buildx bake localbin
$ file ./build/local/bakeme
./build/local/bakeme: Mach-O 64-bit executable arm64
```
