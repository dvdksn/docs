...
sidebar:
  label: Verify an image or chart
  order: 40
---
...
> [create one](../../accounts/create-account/) for free.
>
...
<TabItem label="regctl">

First, authenticate to both registries. Prepare a [personal access token
(PAT)](../../security/access-tokens/) for your user with `read only` access:
...
attestations](./hardened-packages/#package-attestations).
...
See [available
attestations](../core-concepts/attestations/#image-attestations) for a list
of attestations available for each DHI image and [Helm chart
attestations](../core-concepts/attestations/#helm-chart-attestations) for a
...
variant](./explore/#image-variant-details). The **Attestations** section
