...
sidebar:
  label: Customize an image or chart
  order: 25
---
...
owner must first [mirror](./mirror/) the DHI repository to your organization
...
the DHI CLI](./cli/#customize-dhi-images).
...
1. Select **Next: Configure** to configure the following image settings:
   ...
   1. Specify the [environment variables](/build/dockerfile/#env) and their
      values that the image will contain.
   1. Add [labels](/build/dockerfile/#label) to the image.
   ...
   1. Select which [user](/build/dockerfile/#user) to run the images as.
   1. Add [`ENTRYPOINT`](/build/dockerfile/#entrypoint) arguments to the
      image. These arguments are appended to the base image's entrypoint.
   1. Add [`CMD`](/build/dockerfile/#cmd) arguments to the image. These
      arguments are appended to the base image's command.
   1. Override the default (`/`) [working
      directory](/build/dockerfile/#workdir) for the image.
...
- Enable immutable tags: Consider enabling [immutable
  tags](../../docker-hub/repos/manage/hub-images/immutable-tags/) for your
...
To customize image references, an organization owner must [mirror](./mirror/)
