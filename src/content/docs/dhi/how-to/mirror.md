...
sidebar:
  label: Mirror a repository
  order: 20
---


<SummaryBar featureName="Docker Hardened Images" />
...
- [Mirror to Docker Hub](#mirror-a-dhi-repository-to-docker-hub): Mirror a DHI
  ...
  chart](./customize/) and access compliance variants and ELS variants
...
- Image repositories: Mirroring lets you customize images by adding packages,
  ...
  [Customize a Docker Hardened Image](./customize/#customize-a-docker-hardened-image).
...
- Chart repositories: Mirroring lets you customize image references within
  ...
  Hardened Helm chart](./customize/#customize-a-docker-hardened-helm-chart).
...
details, see [Use the DHI CLI](./cli/#mirror-dhi-images).
...
(OAT)](../../enterprise/security/access-tokens/) instead of a personal access
...
1. Set environment variables for your specific environment. Replace the
   ...
   [personal access token (PAT)](../../security/access-tokens/) for the user
   with `read only` access. Alternatively, you can use your organization name and
   an [organization access token
   (OAT)](../../enterprise/security/access-tokens/) to authenticate with `docker.io`.
...
After mirroring, see [Pull a DHI](./use/#pull-a-dhi) to learn how to pull and use mirrored images.
