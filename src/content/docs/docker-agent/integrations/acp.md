...
Desktop or Claude Code, see [MCP integration](./mcp/) instead.
...
- **Docker Agent installed** - See the [installation guide](../#installation)
- **Agent configuration** - A YAML file defining your agent. See the
  [tutorial](../tutorial/) or [example
...
2. Start a new external agent thread. Select your agent in the drop-down list.
   ...
   ![New external thread with Docker Agent in Zed](/docker-agent/images/cagent-acp-zed.avif)
...
are not supported, so [toolsets](../reference/toolsets/) like `shell` or
...
agents](../sharing-agents/) for details on using OCI registries.
...
- Review the [configuration reference](../reference/config/) for advanced
  agent setup
- Explore the [toolsets reference](../reference/toolsets/) to learn what tools
  are available
- Add [RAG for codebase search](../rag/) to your agent
- Check the [CLI reference](../reference/cli/) for all `docker agent serve acp` options
- Browse [example
