...
- Learn [best practices](best-practices/) for handling large outputs,
  structuring agent teams, and optimizing performance
- Integrate Docker Agent with your [editor](integrations/acp/) or use agents as
  [tools in MCP clients](integrations/mcp/)
- Review the [Configuration reference](reference/config/) for all available
  options
- Explore the [Tools reference](reference/toolsets/) to see what capabilities
  you can enable
- Check out [example
