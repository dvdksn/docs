...
[automated tests](automated-testing/) configured, the new image is only pushed
...
3. Select either GitHub or Bitbucket to connect where the image's source code is stored.
   ...
   > service](link-source/). Otherwise, if you are editing the build settings
...
5. Optional. Enable [autotests](automated-testing/#enable-automated-tests-on-a-repository).
...
9. For each branch or tag, enable or disable the **Build Caching** toggle.
    ...
    [Build caching](/manuals/build/building/best-practices/#leverage-build-cache)
...
> [Advanced options for autobuild and autotest](advanced/#source-repository-or-branch-clones)
...
3. Enter the name of the **Source** branch or tag you want to build.
    ...
    > names to build. To learn more, see [regexes](index/#regexes-and-automated-builds).
...
4. Enter the tag to apply to Docker images built from this source.
   ...
   > If you configured a regex to select the source, you can reference the
   > capture groups and use its result as part of the tag. To learn more, see
   > [regexes](index/#regexes-and-automated-builds).
...
Docker build system, add the [environment variable](index/#environment-variables-for-builds)
`DOCKER_BUILDKIT=0`. Refer to the [BuildKit](/manuals/build/buildkit/)
...
variable to automated builds associated with the account. For more information, see [Troubleshoot](troubleshoot/#build-repositories-with-linked-private-submodules)
...
5. Sign in to Docker Hub as an owner, switch to the organization, and follow the instructions to [link to source code repository](link-source/) using the service account.
...
6. Optional. Use the SSH keys you generated to set up any builds with private submodules, using the service account and [the previous instructions](troubleshoot/#build-repositories-with-linked-private-submodules).

## What's Next?

- [Customize your build process](advanced/) with environment variables, hooks, and more
- [Add automated tests](automated-testing/)
- [Manage your builds](manage-builds/)
