...
<tbody>
<tr><th>Description</th><td>Create a volume</td></tr>
<tr><th>Usage</th><td><code>docker volume create [OPTIONS] [VOLUME]</code></td></tr>
</tbody></table>
...
|--------|---------|-------------|
| `--availability` | `active` | **API 1.42+** Cluster Volume availability (`active`, `pause`, `drain`) |
| `-d`, `--driver` | `local` | Specify volume driver name |
| `--group` |  | **API 1.42+** Cluster Volume group (cluster volumes) |
| `--label` |  | Set metadata for a volume |
| `--limit-bytes` |  | **API 1.42+** Minimum size of the Cluster Volume in bytes |
| [`-o`, `--opt`](#opt) |  | Set driver specific options |
| `--required-bytes` |  | **API 1.42+** Maximum size of the Cluster Volume in bytes |
| `--scope` | `single` | **API 1.42+** Cluster Volume access scope (`single`, `multi`) |
| `--secret` |  | **API 1.42+** Cluster Volume secrets |
| `--sharing` | `none` | **API 1.42+** Cluster Volume access sharing (`none`, `readonly`, `onewriter`, `all`)  |
| `--topology-preferred` |  | **API 1.42+** A topology that the Cluster Volume would be preferred in |
| `--topology-required` |  | **API 1.42+** A topology that the Cluster Volume must be accessible from |
| `--type` | `block` | **API 1.42+** Cluster Volume access type (`mount`, `block`) |
