...
After you
[install Docker](/get-started/get-docker/), you can
[install the Go or Python SDK](index/#install-the-sdks) and
...
$ curl --unix-socket /var/run/docker.sock -H "Content-Type: application/json" \
  ...
  -X POST http://localhost/v<Version name="latest_engine_api_version" />/containers/create
{"Id":"1c6594faf5","Warnings":null}

$ curl --unix-socket /var/run/docker.sock -X POST http://localhost/v<Version name="latest_engine_api_version" />/containers/1c6594faf5/start

$ curl --unix-socket /var/run/docker.sock -X POST http://localhost/v<Version name="latest_engine_api_version" />/containers/1c6594faf5/wait
{"StatusCode":0}

$ curl --unix-socket /var/run/docker.sock "http://localhost/v<Version name="latest_engine_api_version" />/containers/1c6594faf5/logs?stdout=1"
hello world
```
...
> for example: `http:/v<Version name="latest_engine_api_version" />/containers/1c6594faf5/start`.
...
```console
$ curl --unix-socket /var/run/docker.sock http://localhost/v<Version name="latest_engine_api_version" />/containers/json
[{
...
$ curl --unix-socket /var/run/docker.sock \
  -X POST http://localhost/v<Version name="latest_engine_api_version" />/containers/ae63e8b89a26/stop
```
...
```console
$ curl --unix-socket /var/run/docker.sock "http://localhost/v<Version name="latest_engine_api_version" />/containers/ca5f55cdb/logs?stdout=1"
...
```console
$ curl --unix-socket /var/run/docker.sock http://localhost/v<Version name="latest_engine_api_version" />/images/json
[{
...
```console
$ curl --unix-socket /var/run/docker.sock \
  -X POST "http://localhost/v<Version name="latest_engine_api_version" />/images/create?fromImage=alpine"
...
$ curl --unix-socket /var/run/docker.sock\
  -X POST "http://localhost/v<Version name="latest_engine_api_version" />/commit?container=0888269a9d&repo=helloworld"
