...
<SummaryBar featureName="Composefile include" />
...
Compose also supports the use of interpolated variables with `include`. It's recommended that you [specify mandatory variables](interpolation/). For example:
...
- A list of strings: When multiple Compose files need to be [merged together](merge/) to define the Compose model for the local application.
...
For more information on using `include`, see [Working with multiple Compose files](/manuals/compose/how-tos/multiple-compose-files/)
