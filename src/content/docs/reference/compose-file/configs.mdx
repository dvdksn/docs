...
import Include_compose_configs from "@includes/compose/configs.mdx";

<Include_compose_configs />

Services can only access configs when explicitly granted by a [`configs`](services/#configs) attribute within the `services` top-level element.
...
The following
example modifies the previous one to look up a config using the parameter `HTTP_CONFIG_KEY`. The actual lookup key is set at deployment time by the [interpolation](interpolation/) of
