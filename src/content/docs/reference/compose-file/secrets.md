...
Secrets are a flavor of [Configs](configs/) focusing on sensitive data, with specific constraint for this usage.

Services can only access secrets when explicitly granted by a [`secrets` attribute](services/#secrets) within the `services` top-level element.
...
For more information, see [How to use secrets in Compose](/manuals/compose/how-tos/use-secrets/).
