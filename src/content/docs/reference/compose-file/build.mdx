...
import Include_compose_build from "@includes/compose/build.mdx";
import SummaryBar from "@components/SummaryBar.astro";

<Include_compose_build />
...
When Compose is confronted with both a `build` subsection for a service and an `image` attribute, it follows the rules defined by the [`pull_policy`](services/#pull_policy) attribute.
...
<SummaryBar featureName="Build additional contexts" />
...
<SummaryBar featureName="Build dockerfile inline" />
...
<SummaryBar featureName="Build entitlements" />
...
`extra_hosts` adds hostname mappings at build-time. Use the same syntax as [`extra_hosts`](services/#extra_hosts).
...
`isolation` specifies a build’s container isolation technology. Like [isolation](services/#isolation), supported values
...
`platforms` defines a list of target [platforms](services/#platform).
...
<SummaryBar featureName="Build privileged" />
...
<SummaryBar featureName="Compose provenance" />
...
<SummaryBar featureName="Compose sbom" />
...
`secrets` grants access to sensitive data defined by [secrets](services/#secrets) on a per-service build basis. Two
different syntax variants are supported: the short syntax and the long syntax.

Compose reports an error if the secret isn't defined in the
[`secrets`](secrets/) section of this Compose file.
...
Such grant must be explicit within service specification as [secrets](services/#secrets) service element.
...
as an integer value representing the number of bytes or as a string expressing a [byte value](extension/#specifying-byte-values).
...
the `image` [property defined in the service section](services/#image)
...
<SummaryBar featureName="Build ulimits" />
