...
import Include_compose_volumes from "@includes/compose/volumes.mdx";

<Include_compose_volumes />

To use a volume across multiple services, you must explicitly grant each service access by using the [volumes](services/#volumes) attribute within the `services` top-level element. The `volumes` attribute has additional syntax that provides more granular control.
