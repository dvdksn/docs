...
import Include_compose_merge from "@includes/compose/merge.mdx";
import SummaryBar from "@components/SummaryBar.astro";

<Include_compose_merge />
...
When merging Compose files that use the services attributes [command](services/#command), [entrypoint](services/#entrypoint) and [healthcheck: `test`](services/#healthcheck), the value is overridden by the latest Compose file, and not appended.
...
Applies to the [ports](services/#ports), [volumes](services/#volumes), [secrets](services/#secrets) and [configs](services/#configs) services attributes.
...
<SummaryBar featureName="Compose replace file" />
...
For more information on how merge can be used to create a composite Compose file, see [Working with multiple Compose files](/manuals/compose/how-tos/multiple-compose-files/)
