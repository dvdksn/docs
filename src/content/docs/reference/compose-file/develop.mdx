...
import Include_compose_develop from "@includes/compose/develop.mdx";
...
Compose to monitor source code for changes. For more information, see [Use Compose Watch](/manuals/compose/how-tos/file-watch/).
...
<SummaryBar featureName="Compose exec" />

`exec` is only relevant when `action` is set to `sync+exec`. Like [service hooks](services/#post_start), `exec` is used to define the command to be run inside the container once it has started.
