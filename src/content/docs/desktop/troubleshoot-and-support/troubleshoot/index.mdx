...
sidebar:
  label: Troubleshoot and diagnose
  order: 10
---
...
- Select the Docker menu Docker menu <img src="/desktop/images/whale-x.svg" alt="whale menu" style="display:inline;height:1.2em;vertical-align:middle;" /> and then **Troubleshoot**.
...
- **Reset Kubernetes cluster**. Select to delete all stacks and Kubernetes resources. For more information, see [Kubernetes](/manuals/desktop/settings-and-maintenance/settings/#kubernetes).
...
4. Use your diagnostics ID to get help:
   - If you have a paid Docker subscription, select **Contact support**. This opens the Docker Desktop support form. Fill in the information required and add the ID you copied in step three to the **Diagnostics ID field**. Then, select **Submit ticket** to request Docker Desktop support.
     > [!NOTE]
     >
     > You must be signed in to Docker Desktop to access the support form. For information on what's covered as part of Docker Desktop support, see [Support](/manuals/support/).
...
> [!TIP]
>
> You can also use the [`docker desktop diagnose` command](/manuals/desktop/features/desktop-cli/) to diagnose Docker Desktop and upload the diagnostics ID.
...
![Mac Console search for Docker app](/desktop/images/console.png)
...
Refer to the [Read the daemon logs](/manuals/engine/daemon/logs/) section
to learn how to view the Docker Daemon logs.

## Further resources

- View specific [troubleshoot topics](topics/).
- View information on [known issues](known-issues/)
- [Fix "Docker.app is damaged" on macOS](mac-damaged-dialog/) - Resolve macOS installation issues
- [Get support for Docker products](/manuals/support/)
