...
<SummaryBar featureName="Wasm workloads" />
...
Wasm workloads require the [containerd image store](containerd/)
...
store](./containerd/), an error similar to the following displays:

```text
docker: Error response from daemon: Unknown runtime specified io.containerd.wasmedge.v1.
```

[Turn on the containerd feature](./containerd/#enable-the-containerd-image-store)
