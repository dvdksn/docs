...
![Overview of the three components of an extension](/desktop/extensions/extensions-sdk/architecture/images/extensions-architecture.png)
...
`ui` section in the `metadata.json`. See the [ui metadata section](metadata/#ui-section) for more details.
...
Learn more about [building a frontend](/desktop/extensions/extensions-sdk/build/frontend-extension-tutorial/) for your extension.
...
attaches it to the Compose project. See the [ui metadata section](metadata/#vm-section) for more details.
...
> See [How to use the Docker socket](../guides/use-docker-socket-from-backend/) from the backend.

To communicate with the backend, the Extension SDK provides [functions](../dev/api/backend/#get) to make `GET`,
...
![Backend and frontend communication](/desktop/extensions/extensions-sdk/architecture/images/extensions-arch-2.png)

Finally, the backend can be built with any technology, as long as it can run in a container and listen on a socket.

Learn more about [adding a backend](/desktop/extensions/extensions-sdk/build/backend-extension-tutorial/) to your extension.
...
that are installed on the host when the extension is installed. The frontend can invoke them with [the extension SDK](../dev/api/backend/#invoke-an-extension-binary-on-the-host).
...
the `metadata.json`. See the [ui metadata section](metadata/#host-section) for more details.

![Executable and frontend communication](/desktop/extensions/extensions-sdk/architecture/images/extensions-arch-3.png)
...
See the [host metadata section](metadata/#host-section) for more details.

Learn how to [invoke host binaries](../guides/invoke-host-binaries/).
