...
Docker Extensions' private marketplace is designed specifically for organizations who don’t give developers root access to their machines. It makes use of [Settings Management](/admin/enterprise/security/hardened-desktop/settings-management/) so administrators have complete control over the private marketplace.
...
> [!IMPORTANT]
>
> If your org is using [Settings Management](/admin/enterprise/security/hardened-desktop/settings-management/) via the [Admin Console](manuals/enterprise/security/hardened-desktop/settings-management/configure-admin-console/), you will not need the `admins-settings.json` file. Delete the generated file and keep only the `extensions.txt` file.
...
To find out more information about the `admin-settings.json` file, see [Settings Management](/admin/enterprise/security/hardened-desktop/settings-management/).
...
Make sure your developers are signed in to Docker Desktop in order for the private marketplace configuration to take effect. As an administrator, you should [enforce sign-in](/admin/enterprise/security/enforce-sign-in/).
