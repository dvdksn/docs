...
Every extension image must contain a `metadata.json` file at the root of its filesystem that defines the [contents of the extension](../architecture/metadata/).

The Docker image must have several [image labels](labels/), providing information about the extension. See how to use [extension labels](labels/) to provide extension overview information.
...
Extensions that install binaries on the host must also provide Windows binaries in the same extension image. See how to [build a multi-arch image](multi-arch/) for your extension.
