...
sidebar:
  label: HTTP routing with Traefik
---
...
![Architecture diagram showing Traefik routing requests to other containers based on the path of the request](/guides/images/traefik-in-development.webp)
...
1. In the `compose.yaml` file, Traefik is using the following configuration:
   ...
   <TabItem label="Using official image">
...
![An architecture diagram showing several components and the routing between them. Traefik is able to send requests to both non-containerized and containerized workloads](/guides/images/traefik-non-containerized-workload-architecture.webp)
