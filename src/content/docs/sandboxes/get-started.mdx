...
rules later. See [Policies](security/policy/) for a full description of each
option.

> [!NOTE]
> See the [FAQ](faq/) for details on why sign-in is required and what
...
inside the sandbox. See [Credentials](security/credentials/) for details on
...
[Interactive mode](usage/#interactive-mode) for details.

![The sbx interactive dashboard showing sandbox status, resource usage, and network governance controls.](/sandboxes/images/sbx-dashboard.png)
...
See [Branch mode](usage/#branch-mode) for more options, including
...
permitted by default. See [Policies](security/policy/) for the full rule
...
- [Usage guide](usage/) — sandbox management, reconnecting, multiple
  workspaces, port forwarding, and more
- [Agents](agents/) — supported agents and configuration
- [Custom environments](agents/custom-environments/) — build your own sandbox
  images
- [Credentials](security/credentials/) — credential storage and management
- [Workspace trust](security/workspace/) — review agent changes safely
- [Policies](security/policy/) — control outbound access
