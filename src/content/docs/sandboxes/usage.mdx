...
The basic workflow is [`run`](/sandboxes/cli/sbx/run/) to start,
[`ls`](/sandboxes/cli/sbx/ls/) to check status,
[`stop`](/sandboxes/cli/sbx/stop/) to pause, and
[`rm`](/sandboxes/cli/sbx/rm/) to clean up:
...
See [Workspace trust](security/workspace/) for security considerations when
...
[`sbx run`](/sandboxes/cli/sbx/run/) creates the sandbox and attaches you to
...
[custom templates](agents/custom-environments/) let you pre-install tools
...
Sandboxes are [network-isolated](security/isolation/) — your browser or local
tools can't reach a server running inside one by default. Use
[`sbx ports`](/sandboxes/cli/sbx/ports/) to forward traffic from your host into
a running sandbox.
...
[custom template](agents/custom-environments/).
