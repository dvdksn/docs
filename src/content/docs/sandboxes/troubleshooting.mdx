...
[`sbx reset`](/sandboxes/cli/sbx/reset/) to stop all VMs and delete all sandbox
data. Create fresh sandboxes afterwards.

## Agent can't install packages or reach an API

Sandboxes use a [deny-by-default network policy](security/policy/).
...
See [Accessing host services from a sandbox](usage/#accessing-host-services-from-a-sandbox).
...
For agents that use the [credential proxy](security/credentials/), make sure
...
[Monitoring network activity](security/policy/#monitoring)
for details.
