...
> For details, see [Just-in-Time](just-in-time/).
...
<TabItem label="Entra ID (SAML 2.0)">
...
You can assign [Docker roles](../roles-and-permissions/) to
...
| ------------ | ---------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `dockerRole` | `member`, `editor`, or `owner`           | If not set, the user defaults to the `member` role. Setting this attribute overrides the default.<br><br>For role definitions, see [Roles and permissions](../roles-and-permissions/).                                                                         |
...
| `dockerTeam` | Docker `teamName` (e.g., `developers`)   | Provisions the user to the specified team in the default or specified organization. If the team doesn't exist, it is automatically created.<br><br>You can still use [group mapping](group-mapping/) to assign users to multiple teams across organizations.   |
...
1. Setup [SSO](../single-sign-on/connect/) and SCIM first.
...
<TabItem label="Entra ID/Azure AD (SAML 2.0 and OIDC)">
...
<TabItem label="Entra ID/Azure AD (OIDC and SAML 2.0)">
