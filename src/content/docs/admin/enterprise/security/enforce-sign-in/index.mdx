...
sidebar:
  label: Enforce sign-in
  order: 30
---

import SummaryBar from "@components/SummaryBar.astro";

<SummaryBar featureName="Enforce sign-in" />
...
- [Registry key method (Windows only)](methods/#registry-key-method-windows-only)
- [Configuration profiles method (Mac only)](methods/#configuration-profiles-method-mac-only)
- [`.plist` method (Mac only)](methods/#plist-method-mac-only)
- [`registry.json` method (All)](methods/#registryjson-method-all)
...
Enforcing Docker Desktop sign-in and [enforcing SSO](/manuals/enterprise/security/single-sign-on/connect/#optional-enforce-sso) are different features that serve different purposes:
...
- To set up sign-in enforcement, see [Configure sign-in enforcement](/manuals/enterprise/security/enforce-sign-in/methods/).
- To configure SSO enforcement, see [Enforce SSO](/manuals/enterprise/security/single-sign-on/connect/).
