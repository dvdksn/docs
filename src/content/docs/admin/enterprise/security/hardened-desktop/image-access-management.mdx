...
- [Enforce sign-in](/admin/enterprise/security/enforce-sign-in/). Image Access Management only takes effect when users are signed in to Docker Desktop with organization credentials.
- Use [personal access tokens (PATs)](/admin/security/access-tokens/) for authentication (Organization access tokens aren't supported)
...
- Image Access Management only controls access to Docker Hub images. Images from other registries aren't affected by these policies. Use [Registry Access Management](/admin/enterprise/security/hardened-desktop/registry-access-management/) to control access to other registries.
