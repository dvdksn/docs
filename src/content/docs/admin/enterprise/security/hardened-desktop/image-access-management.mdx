...
- [Enforce sign-in](/manuals/enterprise/security/enforce-sign-in/). Image Access Management only takes effect when users are signed in to Docker Desktop with organization credentials.
- Use [personal access tokens (PATs)](/manuals/security/access-tokens/) for authentication (Organization access tokens aren't supported)
...
- Image Access Management only controls access to Docker Hub images. Images from other registries aren't affected by these policies. Use [Registry Access Management](/manuals/enterprise/security/hardened-desktop/registry-access-management/) to control access to other registries.
...
- Layer security controls: Image Access Management works best with [Registry Access Management](registry-access-management/) to control which registries developers can access, [Enhanced Container Isolation](enhanced-container-isolation/) to secure containers at runtime, and [Settings Management](settings-management/) to control Docker Desktop configuration.