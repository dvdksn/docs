...
[Configure containerd shims](/reference/cli/dockerd/#configure-containerd-shims).
...
[Configure runc drop-in replacements](/reference/cli/dockerd/#configure-runc-drop-in-replacements).
...
<SummaryBar featureName="Wasmtime" />
...
1. Turn on the [containerd image store](/manuals/engine/storage/containerd/)
...
- To learn more about the configuration options for container runtimes,
  see [Configure container runtimes](/reference/cli/dockerd/#configure-container-runtimes).
- You can configure which runtime that the daemon should use as its default.
  Refer to [Configure the default container runtime](/reference/cli/dockerd/#configure-the-default-container-runtime).
