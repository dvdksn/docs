...
> packages.
>
> On Windows and Mac, we recommend that you install [Docker Desktop](/desktop/)
...
- Enable `seccomp` security profiles if possible. See
  [Enabling `seccomp` for Docker](../security/seccomp/).

- Enable user namespaces if possible. See the
  [Daemon user namespace options](/engine/cli/dockerd/#daemon-user-namespace-options).
...
> [Docker Desktop](/desktop/) instead.
...
> we recommend that you install [Docker Desktop](/desktop/) instead.
