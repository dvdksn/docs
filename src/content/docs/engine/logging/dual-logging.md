...
keywords: |
  docker, logging, driver, dual logging, dual logging, cache, ring-buffer, configuration
---

## Overview

You can use the `docker logs` command to read container logs regardless of the
configured logging driver or plugin. Docker Engine uses the [`local`](drivers/local/)
