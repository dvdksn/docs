...
<TabItem label="openSUSE and SLES">
...
<TabItem label="CentOS, RHEL, and Fedora">
...
- Only the following storage drivers are supported:
  ...
  - `vfs`
- cgroup is supported only when running with cgroup v2 and systemd. See [Limiting resources](./tips/#limiting-resources).
- Following features are not supported:
  - AppArmor
  - Checkpoint
  - Overlay network
  - Exposing SCTP ports
- To use the `ping` command, see [Routing ping packets](./tips/#routing-ping-packets).
- To expose privileged TCP/UDP ports (&lt; 1024), see [Exposing privileged ports](./tips/#exposing-privileged-ports).
...
For more information, see [Limiting resources](./tips/#limiting-resources).
...
To allow exposing privileged ports, see [Exposing privileged ports](./tips/#exposing-privileged-ports).
...
For details, see [Routing ping packets](./tips/#routing-ping-packets).
