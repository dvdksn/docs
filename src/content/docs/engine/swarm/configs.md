...
Configs operate in a similar way to [secrets](secrets/), except that they are
...
- On Windows, create or update a service using `--credential-spec` with the
  ...
  [Deploy services to a swarm](services/#gmsa-for-swarm).
...
using. See [Rotate a config](configs/#example-rotate-a-config) for a way to
...
[the Compose file reference](/reference/compose-file/configs/) for details.
