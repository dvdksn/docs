...
files. However, Docker supports the use of [configs](configs/)
...
using. See [Rotate a secret](secrets/#example-rotate-a-secret) for a way to
...
[example about using secrets with a service](secrets/#simple-example-get-started-with-secrets).
...
[the Compose file reference](/compose/legacy-versions/) for details.
...
4.  Create an IIS service and grant it access to the `homepage` secret.
    ...
    > example; [configs](configs/) are a better fit. This example is
...
[Compose Specification](/reference/compose-file/secrets/).
