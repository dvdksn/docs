...
Refer to [How nodes work](how-swarm-mode-works/nodes/)
for a brief overview of Docker Swarm mode and the difference between manager and
worker nodes.

## Operate manager nodes in a swarm

Swarm manager nodes use the [Raft Consensus Algorithm](raft/) to manage the
...
information, see [Run Docker Engine in swarm mode](swarm-mode/#configure-the-advertise-address). Because manager nodes are
...
subsystems. Refer to [Add nodes to a swarm](join-nodes/) for more information
...
[Add nodes to the swarm](join-nodes/) to balance your swarm's
...
[nodes API documentation](/engine/api/version/v1.25/#tag/Node)
...
[Join nodes to a swarm](join-nodes/).
...
1.  If the swarm has auto-lock enabled, you need the unlock key
    ...
    [Lock your swarm to protect its encryption key](swarm_manager_locking/).
...
6.  If you use auto-lock,
    [rotate the unlock key](swarm_manager_locking/#rotate-the-unlock-key).
...
If you have configured it, your service uses a [rolling update](swarm-tutorial/rolling-update/).
