...
Once you've [created a swarm](create-swarm/) with a manager node, you're ready
to add worker nodes.
...
2.  Run the command produced by the `docker swarm init` output from the
    [Create a swarm](create-swarm/) tutorial step to create a worker node
...
4.  Run the command produced by the `docker swarm init` output from the
    [Create a swarm](create-swarm/) tutorial step to create a second worker
...
<LinkButton href="deploy-service.md">Deploy a service</LinkButton>
