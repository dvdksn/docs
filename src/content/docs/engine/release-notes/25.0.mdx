...
> [Recursive mounts](/engine/storage/bind-mounts/#recursive-mounts).
...
- Deprecate API versions older than 1.24. [Deprecation notice](../deprecated/#deprecate-legacy-api-versions)
- Deprecate `IsAutomated` field and `is-automated` filter for `docker search`. [Deprecation notice](../deprecated/#isautomated-field-and-is-automated-filter-on-docker-search)
