...
> [containerd image store](../containerd/) by default for fresh installations.
...
> store, see [containerd image store](../containerd/).
...
the [volumes section](../volumes/) to learn how to use volumes to persist data
...
[Best practices for writing Dockerfiles](/manuals/build/building/best-practices/)
and [use multi-stage builds](/manuals/build/building/multi-stage/)
...
![Layers of a container based on the Ubuntu image](/engine/storage/drivers/images/container-layers.webp)
...
![Containers sharing the same image](/engine/storage/drivers/images/sharing-layers.webp)
...
> the exact same data. Refer to the [volumes section](../volumes/) to learn
> about volumes.
...
- Disk space used for log files stored by the [logging-driver](/manuals/engine/logging/).
...
8. Check out the history of each image.
   ...
   images for caching. Refer to [BuildKit](/manuals/build/buildkit/)
...
9. Check out the layers for each image
   ...
   > [format command and log output section](/manuals/engine/cli/formatting/).
...
> layer. Refer to the [use volumes](../volumes/) section to learn about
> volumes.
...
2. Run the `docker ps` command with the `--size` option to verify the 5 containers
   ...
   <details>
<summary>Advanced: metadata and logs storage used for containers</summary>
...
work, with one or more virtual disks per virtual machine. The [`vfs` storage](vfs-driver/)
...
* [Volumes](../volumes/)
* [Select a storage driver](select-storage-driver/)
