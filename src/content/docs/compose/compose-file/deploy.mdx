...
import Include_compose_deploy from "@includes/compose/deploy.mdx";

<Include_compose_deploy />
...
`memory` configures a limit or reservation on the amount of memory a container can allocate, set as a string expressing a [byte value](extension/#specifying-byte-values).
...
- `delay`: How long to wait between restart attempts, specified as a [duration](extension/#specifying-durations). The default is 0, meaning restart attempts can occur immediately.
...
- `window`: The amount of time to wait after a restart to determine whether it was successful, specified as a [duration](extension/#specifying-durations) (default: the result is evaluated immediately after the restart).
