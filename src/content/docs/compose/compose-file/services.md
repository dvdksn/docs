...
import Include_compose_services from "@includes/compose/services.mdx";
import Include_compose_services_depends_on from "@includes/compose/services-depends-on.mdx";
import Include_compose_services_env_file from "@includes/compose/services-env-file.mdx";
import Include_compose_services_environment from "@includes/compose/services-environment.mdx";
import Include_compose_services_healthcheck from "@includes/compose/services-healthcheck.mdx";
import Include_compose_services_networks from "@includes/compose/services-networks.mdx";
import Include_compose_services_ports from "@includes/compose/services-ports.mdx";
import Include_compose_services_secrets from "@includes/compose/services-secrets.mdx";
import Include_compose_services_volumes from "@includes/compose/services-volumes.mdx";
import SummaryBar from "@components/SummaryBar.astro";

<Include_compose_services />
...
described in detail in the [Compose Build Specification](build/) documentation.
...
described in detail in the [Compose Deploy Specification](deploy/) documentation.
...
<SummaryBar featureName="Compose attach" />
...
`build` specifies the build configuration for creating a container image from source, as defined in the [Compose Build Specification](build/).
...
an integer value using microseconds as unit or a [duration](extension/#specifying-durations).
...
When set, `cpus` must be consistent with the `cpus` attribute in the [Deploy Specification](deploy/#cpus).
...
<SummaryBar featureName="Compose cgroup" />
...
> [!NOTE]
>
> Unlike the `CMD` instruction in a Dockerfile, the `command` field doesn't automatically run within the context of the [`SHELL`](/build/dockerfile/#shell-form) instruction defined in the image. If your `command` relies on shell-specific features, such as environment variable expansion, you need to explicitly run it within a shell. For example:
>
> ```yaml
> command: /bin/sh -c 'echo "hello $$HOSTNAME"'
> ```

The value can also be a list, similar to the [exec-form syntax](/build/dockerfile/#exec-form)
used by the [Dockerfile](/build/dockerfile/#exec-form).
...
[`configs` top-level element](configs/) in the Compose file.
...
<Include_compose_services_depends_on />
...
`deploy` specifies the configuration for the deployment and lifecycle of services, as defined [in the Compose Deploy Specification](deploy/).

### `develop`

<SummaryBar featureName="Compose develop" />

`develop` specifies the development configuration for maintaining a container in sync with source, as defined in the [Development Section](develop/).
...
<SummaryBar featureName="Compose driver opts" />
...
[Dockerfile](https://docs.docker.com/build/dockerfile/#cmd):
...
<Include_compose_services_env_file />
...
<SummaryBar featureName="Compose required" />
...
<SummaryBar featureName="Compose format" />
...
<Include_compose_services_environment />
...
<SummaryBar featureName="Compose gpus" />

`gpus` specifies GPU devices to be allocated for container usage. This is equivalent to a [device request](deploy/#devices) with
...
<Include_compose_services_healthcheck />

For more information on `HEALTHCHECK`, see the [Dockerfile reference](/build/dockerfile/#healthcheck).
...
`interval`, `timeout`, `start_period`, and `start_interval` are [specified as durations](extension/#specifying-durations). Introduced in Docker Compose version [2.20.2](https://github.com/docker/compose/releases/tag/v2.20.2)
...
If you are also using the [Compose Build Specification](build/), there are alternative options for controlling the precedence of
...
<SummaryBar featureName="Compose label file" />
...
`mem_limit` configures a limit on the amount of memory a container can allocate, set as a string expressing a [byte value](extension/#specifying-byte-values).

When set, `mem_limit` must be consistent with the `limits.memory` attribute in the [Deploy Specification](deploy/#memory).

### `mem_reservation`

`mem_reservation` configures a reservation on the amount of memory a container can allocate, set as a string expressing a [byte value](extension/#specifying-byte-values).

When set, `mem_reservation` must be consistent with the `reservations.memory` attribute in the [Deploy Specification](deploy/#memory).
...
attribute that only has meaning if [`memory`](deploy/#memory) is also set. Using swap lets the container write excess
...
<SummaryBar featureName="Compose models" />

`models` defines which AI models the service should use at runtime. Each referenced model must be defined under the [`models` top-level element](models/).
...
<Include_compose_services_networks />
...
services:
  some-service:
    networks:
      ...
      - other-network
```
For more information about the `networks` top-level element, see [Networks](networks/).
...
<SummaryBar featureName="Compose interface-name" />
...
<SummaryBar featureName="Compose mac address" />
...
<SummaryBar featureName="Compose gw priority" />
...
When set, `pids_limit` must be consistent with the `pids` attribute in the [Deploy Specification](deploy/#pids).
...
<Include_compose_services_ports />
...
<SummaryBar featureName="Compose post start" />
...
<SummaryBar featureName="Compose pre stop" />
...
<SummaryBar featureName="Compose provider services" />
...
- `missing`: Compose pulls the image only if it's not available in the platform cache.
   This is the default option if you are not also using the [Compose Build Specification](build/).
...
When both are set, `scale` must be consistent with the `replicas` attribute in the [Deploy Specification](deploy/#replicas).

### `secrets`

<Include_compose_services_secrets />
...
[`secrets` top-level section](secrets/) of the Compose file.
...
Note that support for `uid`, `gid`, and `mode` attributes are not implemented in Docker Compose when the source of the secret is a [`file`](secrets/). This is because bind-mounts used under the hood don't allow uid remapping.
...
It's specified as a [byte value](extension/#specifying-byte-values).
...
as a [duration](extension/#specifying-durations).
...
<SummaryBar featureName="Compose uts" />
...
<Include_compose_services_volumes />
...
For more information about the `volumes` top-level element, see [Volumes](volumes/).
