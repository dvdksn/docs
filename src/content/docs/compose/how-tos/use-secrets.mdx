...
sidebar:
  label: Secrets in Compose
  order: 60
---

import Include_compose_secrets from "@includes/compose/secrets.mdx";

A secret is any piece of data, such as a password, certificate, or API key, that shouldn’t be transmitted over a network or stored unencrypted in a Dockerfile or in your application’s source code.

<Include_compose_secrets />
...
Getting a secret into a container is a two-step process. First, define the secret using the [top-level secrets element in your Compose file](/reference/compose-file/secrets/). Next, update your service definitions to reference the secrets they require with the [secrets attribute](/reference/compose-file/services/#secrets). Compose grants access to secrets on a per-service basis.
...
- [Secrets attribute for services top-level element](/reference/compose-file/services/#secrets)
