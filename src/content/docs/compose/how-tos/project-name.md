...
2. The [COMPOSE_PROJECT_NAME environment variable](environment-variables/envvars/).
3. The [top-level `name:` attribute](/reference/compose-file/version-and-name/) in your Compose file. Or the last `name:` if you [specify multiple Compose files](multiple-compose-files/merge/) in the command line with the `-f` flag.
4. The base name of the project directory containing your Compose file. Or the base name of the first Compose file if you [specify multiple Compose files](multiple-compose-files/merge/) in the command line with the `-f` flag. 
