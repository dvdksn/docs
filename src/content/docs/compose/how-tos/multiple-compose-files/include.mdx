...
import Include_compose_include from "@includes/compose/include.mdx";
import SummaryBar from "@components/SummaryBar.astro";

<SummaryBar featureName="Compose include" />

<Include_compose_include />

The [`include` top-level element](/compose/how-tos/multiple-compose-files/include/) helps to reflect the engineering team responsible for the code directly in the config file's organization. It also solves the relative path problem that [`extends`](extends/) and [merge](merge/) present. 
...
[`include` top-level element](/compose/how-tos/multiple-compose-files/include/)
