...
can either use the pre-defined [COMPOSE_FILE](../environment-variables/envvars/#compose_file) environment variable, or use the `-f` option to specify the list of files. 
...
- You can use the `-f` flag to specify a path to a Compose file that is not located in the current directory, either from the command line or by setting up a [COMPOSE_FILE environment variable](../environment-variables/envvars/#compose_file) in your shell or in an environment file.
...
For more information, see [Using Compose in production](../production/). 
