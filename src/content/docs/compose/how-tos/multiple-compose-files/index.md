...
The quickest way to work with multiple Compose files is to [merge](merge/) Compose files using the `-f` flag in the command line to list out your desired Compose files. However, [merging rules](merge/#merging-rules) means this can soon get quite complicated.
...
- [Extend a Compose file](extends/) by referring to another Compose file and selecting the bits you want to use in your own application, with the ability to override some attributes.
- [Include other Compose files](include/) directly in your Compose file.
