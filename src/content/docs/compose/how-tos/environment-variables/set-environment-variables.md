...
>[!TIP]
>
> Don't use environment variables to pass sensitive information, such as passwords, in to your containers. Use [secrets](../use-secrets/) instead.
...
- You can also take advantage of [interpolation](variable-interpolation/#interpolation-syntax). In the following example, the result is similar to the one above but Compose gives you a warning if the `DEBUG` variable is not set in the shell environment or in an `.env` file in the project directory.
...
- [Understand environment variable precedence](envvars-precedence/).
- [Set or change predefined environment variables](envvars/)
- [Explore best practices](best-practices/)
- [Understand interpolation](variable-interpolation/)
