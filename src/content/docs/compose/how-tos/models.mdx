...
Services can only access models when explicitly granted by a [`models` attribute](services/#models) within the `services` top-level element.
...
For more examples and information on using `model`, see [Use AI models in Compose](/ai-overview/compose/models-and-compose/)