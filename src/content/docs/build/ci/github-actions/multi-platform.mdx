...
sidebar:
  label: Multi-platform image
---

import Version from "@components/Version.astro";

You can build [multi-platform images](../../building/multi-platform/) using
...
[Docker GitHub Builder](github-builder/). The reusable workflows
compute the per-platform matrix, run each platform on its own runner, and
create the final manifest for you.

The following workflow uses the [`build.yml` reusable workflow](github-builder/build/)
...
build inputs directly, see [Build with Docker GitHub Builder build.yml](github-builder/build/).

### With Bake

You can use the [`bake.yml` reusable workflow](github-builder/bake/) for the
...
jobs:
  bake:
    uses: docker/github-builder/.github/workflows/bake.yml@<Version name="github_builder_version" />
    permissions:
      contents: read
      id-token: write
    with:
