...
![GitHub Builder overview](/build/ci/github-actions/github-builder/images/architecture-overview.png)

## Core architecture

A caller workflow invokes either [`build.yml`](build/) or [`bake.yml`](bake/).
[`build.yml`](build/) is the entrypoint for Dockerfile-oriented builds.
[`bake.yml`](bake/) is the entrypoint for Bake-oriented builds, where the
...
![GitHub Builder execution flow](/build/ci/github-actions/github-builder/images/execution-flow.png)
...
[`build.yml`](build/) is the better fit when the build is already expressed as
...
[`bake.yml`](bake/) is the better fit when the repository already uses Bake
