...
FROM golang:<Version name="example_go_version" />
WORKDIR /src
COPY &lt;<EOF ./main.go
package main
...
```dockerfile
# syntax=docker/dockerfile:1
FROM golang:<Version name="example_go_version" /> AS build
WORKDIR /src
...
With [BuildKit enabled](../buildkit/#getting-started), building the
