...
<SummaryBar featureName="Build URL Queries" />
...
```dockerfile
FROM alpine:<Version name="example_alpine_version" />
```
...
```console
docker buildx build --build-context alpine:<Version name="example_alpine_version" />=docker-image://alpine:edge .
```
