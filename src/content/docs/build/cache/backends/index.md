...
`gha` cache backends, but only if you have enabled the [containerd image store](/manuals/desktop/features/containerd/).
Other cache backends require you to select a different [driver](/manuals/build/builders/drivers/).
...
- `inline`: embeds the build cache into the image.
  ...
  This only works with the [`image` exporter](../../exporters/image-registry/).
...
[exporter compression options](../../exporters/#compression). This is
...
[exporter OCI options](../../exporters/#oci-media-types). These are
supported by the `local` and `registry` cache backends.
