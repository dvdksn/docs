...
policy decision for each source. See [Input reference](./inputs/) for all
available fields.
...
If it fails, see [Debugging](./debugging/) for troubleshooting guidance.
...
Validate policies in continuous integration by running builds with the `--policy` flag. For unit testing policies before running builds, see [Test build policies](./testing/).
...
- Browse working examples: [Example policies](./examples/)
- Reference all input fields: [Input reference](./inputs/)
